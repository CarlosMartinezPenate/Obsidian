---
title: Symbiodiniaceae S1 Branching, Scale-Up, Plating, and Archive Plan
aliases:
  - S1 to M1 S2 F2P1 Plan
  - Symbiodiniaceae Scale-Up Plan
date: 2026-08-02
status: planned
tags:
  - protocol
  - Symbiodiniaceae
  - culture
  - scale-up
  - F2
  - IMK
  - colony-isolation
---

# Symbiodiniaceae S1 Branching, Scale-Up, Plating, and Archive Plan

## Objective

Branch each established **S1 F/2 culture** into:

1. **M1** — a fresh maintenance backup;
2. **S2** — a biomass-production culture;
3. **F2P1** — low-density F/2 agar plates for colony recovery and solid-phase backup;
4. **A1** — frozen analytical archive pellets;
5. **H1** — a first pilot harvest from **SSB01** only.

Each branch is derived directly from the original S1 culture::

```text
S1
├──► M1
├──► S2
├──► F2P1
├──► A1
└──► H1 (SSB01 only)
```

> [!important]
> Establish **M1 before harvesting or archiving the source culture**.
>
> Keep the residual S1 cultures as temporary emergency backups until M1 and S2 are clearly established.
>
> Do not use bacterial glycerol-stock conditions for these dinoflagellates. A1 is a **non-viable analytical pellet archive**, not validated cryopreservation.

---

# Culture lineage diagram

**One dot represents one physical culture flask.**

```text
● SSA01-S1
├──► ● SSA01-M1
├──► ● SSA01-S2
├──► ◇ SSA01-F2P1
└──► ◇ SSA01-A1

● SSA02-S1
├──► ● SSA02-M1
├──► ● SSA02-S2
├──► ◇ SSA02-F2P1
└──► ◇ SSA02-A1

● SSA03-S1
├──► ● SSA03-M1
├──► ● SSA03-S2
├──► ◇ SSA03-F2P1
└──► ◇ SSA03-A1

● SSB01-S1
├──► ● SSB01-M1
├──► ● SSB01-S2
├──► ◇ SSB01-F2P1
├──► ◇ SSB01-A1
└──► ◇ SSB01-H1

● SSE01-S1
├──► ● SSE01-M1
├──► ● SSE01-S2
├──► ◇ SSE01-F2P1
└──► ◇ SSE01-A1
```

Legend:

- `●` = one living culture flask
- `◇ F2P1` = low-density F/2 agar plate set
- `◇ A1` = frozen analytical pellet archive
- `◇ H1` = pilot harvest

## Flask count represented above

- S1 source flasks already present: **5**
- New M1 maintenance flasks: **5**
- New S2 production flasks: **5**
- Total new culture flasks required: **10**

---

# Related protocols and recipes

## General preparation

- [[Cleaning Glassware]]
- [[Autoclaving Protocol]]

## F/2 and ASW

- [[ASW recipe]]
- [[F2 Medium recipe]]

## IMK

- [[IMK liquid media recipe]]
- [[10x Daigo's IMK stock recipe]]
- [[Daigo's IMK Medium recipe]]
- [[Daigo's IMK+Glucose recipe - from 50x Daigo's IMK stock]]
- [[Daigos' IMK+Caseine recipe - from 50x Daigo's IMK stock]]

---

# Experimental design

## M1 maintenance cultures

Use a uniform **20 mL inoculum** for all strains to maximize the probability of successful establishment while preserving ample S1 backup volume.

| Strain | Final M1 volume | S1 inoculum | Fresh F/2 | Vessel |
|---|---:|---:|---:|---|
| SSA01 | 100 mL | 20 mL | 80 mL | 250 mL flask |
| SSA02 | 100 mL | 20 mL | 80 mL | 250 mL flask |
| SSA03 | 100 mL | 20 mL | 80 mL | 250 mL flask |
| SSB01 | 100 mL | 20 mL | 80 mL | 250 mL flask |
| SSE01 | 100 mL | 20 mL | 80 mL | 250 mL flask |
| **Total** | **500 mL** | **100 mL** | **400 mL** | **5 flasks** |

## S2 biomass-production cultures

Each strain is maintained in **one 2 L Erlenmeyer flask**.

The 1 L cultures retain approximately 1 L headspace, while the 500 mL cultures retain approximately 1.5 L headspace.

| Strain | S2 flask | Final volume | S1 inoculum | Fresh F/2 | Vessel |
|---|---|---:|---:|---:|---|
| SSA01 | S2 | 500 mL | 100 mL | 400 mL | 2 L flask |
| SSA02 | S2 | 1,000 mL | 100 mL | 900 mL | 2 L flask |
| SSA03 | S2 | 1,000 mL | 100 mL | 900 mL | 2 L flask |
| SSB01 | S2 | 1,000 mL | 100 mL | 900 mL | 2 L flask |
| SSE01 | S2 | 500 mL | 100 mL | 400 mL | 2 L flask |
| **Total** |  | **4,000 mL** | **500 mL** | **3,500 mL** | **5 flasks** |

## Combined liquid F/2 requirement

| Use | Final culture volume | S1 inoculum | Fresh F/2 required |
|---|---:|---:|---:|
| M1 | 500 mL | 100 mL | 400 mL |
| S2 | 4,000 mL | 500 mL | 3,500 mL |
| **Total** | **4,500 mL** | **600 mL** | **3,900 mL** |

Prepare **4.0 L of fresh 1× F/2**, leaving approximately **100 mL reserve**.

Using 50× F/2 stock:

- **80 mL 50× F/2 stock**
- **3,920 mL sterile ASW**
- **4,000 mL final 1× F/2**

See [[F2 Medium recipe]].

## S1 source-volume balance

Assuming each S1 flask contains approximately **200 mL**:

| Strain | Starting S1 | M1 inoculum | S2 inoculum | Estimated remaining S1 |
|---|---:|---:|---:|---:|
| SSA01 | 200 mL | 20 mL | 100 mL | 80 mL |
| SSA02 | 200 mL | 20 mL | 100 mL | 80 mL |
| SSA03 | 200 mL | 20 mL | 100 mL | 80 mL |
| SSB01 | 200 mL | 20 mL | 100 mL | 80 mL |
| SSE01 | 200 mL | 20 mL | 100 mL | 80 mL |
| **Total** | **1,000 mL** | **100 mL** | **500 mL** | **400 mL** |

Keep the residual S1 cultures as temporary backups for **2–4 weeks**, or until M1 and S2 clearly recover.

For **SSB01**, the approximately 80 mL remaining after M1 and S2 establishment may be allocated as follows:

- 5 mL for microscopy, OD, and plate preparation;
- 3 × 5 mL for A1 analytical pellets;
- approximately 50 mL for H1 pilot harvest;
- approximately 10 mL retained temporarily as an S1 backup.

> [!caution]
> Measure the actual S1 volume before beginning.
>
> If any source culture contains less than approximately 150 mL, establish M1 first and reduce or postpone that strain’s S2 scale-up rather than consuming the entire source.

---

# Materials and inventory

## Core S1 branching experiment

### Media

Liquid cultures require **3.9 L** fresh F/2; prepare **4.0 L** to provide approximately 100 mL reserve.

The plate calculation below assumes **300 mL F/2 agar**, which is exactly 25 mL per plate. Prepare extra plate medium if losses during pouring are expected.

- [ ] **4.0 L fresh liquid 1× F/2**
- [ ] **300 mL 1× F/2 agar**
- [ ] **4.214 L sterile ASW total**
- [ ] **86 mL 50× F/2 stock total**
- [ ] Agar according to [[F2 Medium recipe]]
- [ ] Sterile ASW or sterile F/2 for dilution blanks

Breakdown:

| Use | 50× F/2 stock | Sterile ASW | Final volume |
|---|---:|---:|---:|
| Liquid F/2 | 80 mL | 3,920 mL | 4,000 mL |
| F/2 agar | 6 mL | 294 mL | 300 mL |
| **Total** | **86 mL** | **4,214 mL** | **4,300 mL** |

### Glassware

- [ ] **5 × 250 mL Erlenmeyer flasks** for M1
- [ ] **5 × 2 L Erlenmeyer flasks** for S2
- [ ] Sterile gauze closures or suitable sterile flask covers for 10 flasks
- [ ] Aluminum foil
- [ ] Autoclave tape

Clean and sterilize according to:

- [[Cleaning Glassware]]
- [[Autoclaving Protocol]]

### Low-density F/2 plating

- [ ] **12 sterile Petri plates**
- [ ] **10 sterile dilution tubes**: two per strain for 10⁻¹ and 10⁻² dilutions
- [ ] Sterile spreaders or sterile beads
- [ ] Sterile pipette tips
- [ ] Plate-sealing film or bags, if used by the lab

### Archive and harvest

- [ ] **15 labeled cryovials** for A1 analytical pellet archives: three per strain
- [ ] **At least 15 × 15 mL tubes** for sampling and archive preparation
- [ ] **At least 2 × 50 mL tubes** for the SSB01 pilot harvest
- [ ] Additional 50 mL tubes if the harvest volume is larger than expected
- [ ] Liquid nitrogen-compatible tools only if flash-freezing pellets
- [ ] Cryobox and −80°C storage position

### General consumables

- [ ] Sterile serological pipettes: 1, 5, 10, 25, and 50 mL as needed
- [ ] Sterile micropipette tips
- [ ] Labels resistant to incubator humidity and −80°C
- [ ] Permanent marker
- [ ] 70% ethanol
- [ ] Gloves
- [ ] Refractometer
- [ ] OD cuvettes or plate-reader consumables
- [ ] Microscope slides and coverslips

---

# Optional IMK optimization module

This module is separate from the core F/2 scale-up and may be started later.

## Conditions

| Strain | Plain IMK | IMK + casein | IMK + glucose | Total flasks |
|---|---:|---:|---:|---:|
| SSA01 | 50 mL | 50 mL | 50 mL | 3 |
| SSA02 | 50 mL | 50 mL | 50 mL | 3 |
| SSA03 | 50 mL | 50 mL | — | 2 |
| SSE01 | 50 mL | — | — | 1 |
| **Total** | **200 mL** | **150 mL** | **100 mL** | **9** |

SSB01 is excluded from this first IMK screen because it is prioritized for F/2 biomass production and the pilot harvest.

## Optional IMK materials

For **450 mL final IMK-based medium** prepared from 50× stock:

- **9 mL 50× Daigo’s IMK stock**
- **441 mL sterile ASW**
- **0.60 g casein hydrolysate** for 150 mL IMK + casein
- **0.50 g glucose** for 100 mL IMK + glucose
- **9 × 250 mL flasks**

Recipes:

- [[Daigo's IMK Medium recipe]]
- [[Daigo's IMK+Glucose recipe - from 50x Daigo's IMK stock]]
- [[Daigos' IMK+Caseine recipe - from 50x Daigo's IMK stock]]

> [!note]
> If the optional IMK module is run together with the core F/2 work, prepare at least **4.655 L sterile ASW total**.
>
> At 37.1 g Coral Pro per liter, 4.655 L ASW begins with approximately **172.7 g Coral Pro**, followed by salinity measurement and adjustment according to [[ASW recipe]].

---

# Low-density F/2 agar plating

The purpose of F2P1 is to establish a **solid-phase colony-recovery backup**, not to prove axenicity.

## Plate layout

Prepare two plates per strain:

| Plate code | Inoculum |
|---|---|
| F2P1-10-1 | 100 µL of 10⁻¹ dilution |
| F2P1-10-2 | 100 µL of 10⁻² dilution |

For five strains:

- 5 strains × 2 plates = **10 inoculated plates**
- 2 uninoculated F/2 agar controls = **2 plates**
- **Total = 12 plates**

> [!note]
> Because these cultures are strongly clumped, some colonies may originate from small aggregates rather than single cells.

## Incubation conditions

- Temperature: **24–26°C**
- Photoperiod: **12 h light : 12 h dark**
- Light: approximately **20–30 µmol photons m⁻² s⁻¹**
- Protect plates from drying
- Inspect weekly
- Expected colony-development period: approximately **2–4 weeks**, possibly longer

## Follow-up after colony formation

1. Identify discrete pigmented colonies.
2. Pick one colony into a small fresh liquid F/2 culture.
3. Transfer part of the same colony to a fresh F/2 agar plate.
4. Keep the original plate until both daughter cultures establish.

The resulting lineage is:

```text
F2P1 colony
├──► liquid colony-derived backup
└──► fresh solid F/2 backup
```
---
# Labeling scheme

## Living cultures

```text
STRAIN-GENERATION-DATE
```

Examples:

```text
SSA01-M1-2026-08-02
SSA02-S2-2026-08-02
SSB01-S2-2026-08-02
```

## Plates

```text
STRAIN-F2P1-DILUTION-DATE
```

Examples:

```text
SSA03-F2P1-10-1-2026-08-02
SSA03-F2P1-10-2-2026-08-02
```

## Archive pellets

```text
STRAIN-A1-REPLICATE-DATE
```

Example:

```text
SSE01-A1-Rep1-2026-08-02
```

## Residual S1 backup

```text
STRAIN-S1-BACKUP-DATE
```

Example:

```text
SSA02-S1-BACKUP-2026-08-02
```

---

# Pre-work preparation

## One or more days before inoculation

1. Confirm that all S1 cultures are alive by microscopy.
2. Record:
   - OD750;
   - culture color;
   - visible clumping or sediment;
   - chlorophyll autofluorescence, when available;
   - approximate source volume.
3. Clean the required glassware according to [[Cleaning Glassware]].
4. Prepare gauze closures.
5. Sterilize dry flasks and closures according to [[Autoclaving Protocol]].
6. Prepare sterile ASW according to [[ASW recipe]].
7. Prepare 4.0 L fresh F/2 according to [[F2 Medium recipe]].
8. Prepare and pour 12 low-density F/2 agar plates.
9. Label all flasks, plates, dilution tubes, archive tubes, and harvest tubes before beginning culture transfers.
10. Confirm incubator space for:
    - 5 M1 flasks;
    - 5 S2 flasks;
    - 5 remaining S1 source flasks;
    - 12 F/2 agar plates.

---

# Aseptic branching procedure

## 1. Arrange the work

Process one strain at a time in this order:

```text
SSA01 → SSA02 → SSA03 → SSB01 → SSE01
```

For each strain, complete transfers in this order:

```text
M1 → S2 → F2P1 → A1 → H1 (SSB01 only)
```

Use a new sterile pipette for each strain and for each transfer type when cross-contamination is possible.

---

## 2. Mix the S1 source culture

1. Inspect the source culture before opening.
2. Gently swirl or invert as appropriate to distribute cells.
3. Do not vortex.
4. For strongly clumped SSB01, mix consistently but avoid aggressive mechanical disruption unless the dispersed-versus-clumped condition is being tested deliberately.
5. Remove a small microscopy sample after mixing.

---

## 3. Establish M1

For every strain:

1. Add **80 mL** fresh F/2 to a sterile 250 mL flask.
2. Add **20 mL** of the corresponding S1 culture.
3. Mix gently by swirling.
4. Close with a sterile breathable closure.
5. Return the flask to the incubator.

---

## 4. Establish S2

### SSA01 and SSE01

1. Add 400 mL fresh F/2 to one labeled 2 L flask.
2. Add 100 mL of the corresponding S1 culture.
3. Mix gently.

### SSA02, SSA03, and SSB01

For each strain:

1. Add 900 mL fresh F/2 to one labeled 2 L flask.
2. Add 100 mL S1 inoculum.
3. Mix gently.

---

## 5. Prepare F2P1 plates

1. Mix the source culture thoroughly but gently.
2. Prepare 10⁻¹ and 10⁻² dilutions using sterile F/2 or sterile ASW.
3. Plate 100 µL of the 10⁻¹ dilution onto **F2P1-10-1**.
4. Plate 100 µL of the 10⁻² dilution onto **F2P1-10-2**.
5. Prepare **one shared pair of uninoculated control plates for the complete plating batch**.
6. Allow the inoculum to absorb completely.
7. Incubate under the conditions described below.
8. Record the inoculation date and plate IDs.

---

## 6. Prepare A1 analytical pellet archives

For each strain:

1. Aliquot a defined culture volume into three labeled tubes.
2. Centrifuge using a validated setting that pellets the cells without unnecessary damage.
3. Remove the supernatant carefully.
4. Record the culture volume represented by each pellet.
5. Flash-freeze the closed pellet tubes if that is the laboratory-approved practice.
6. Store at −80°C.
7. Record the freezer, rack, box, and position.

> [!warning]
> These pellets are intended for later DNA, pigment, or protein analysis. They are **not** validated viable cryostocks.

---

## 7. Retain residual S1 cultures

The residual S1 cultures serve as emergency backups.

1. Leave each S1 culture in its original flask whenever possible.
2. Do not routinely add fresh medium.
3. Label the flask as **S1-BACKUP** if it must be transferred.
4. Retain for approximately **2–4 weeks**.
5. Retire, archive, or harvest S1 after:
   - M1 is established;
   - S2 is actively growing;
   - no major contamination or transfer failure is evident.
6. F2P1 colony recovery may continue independently after S1 retirement.

For **SSB01**, the approximately **80 mL** remaining after M1 and S2 establishment may be allocated as follows:

- 5 mL for microscopy, OD750, and plate preparation;
- 3 × 5 mL for A1 analytical pellets;
- approximately 50 mL for the H1 pilot harvest;
- approximately 10 mL retained temporarily as an S1 backup.

> [!caution]
> Measure the actual S1 volume before beginning. If any source culture contains less than approximately **150 mL**, establish M1 first and reduce or postpone that strain's S2 scale-up rather than consuming the entire source.

---

## 8. Pilot harvest of SSB01

Only begin after **SSB01-M1** and **SSB01-S2** have been established.

1. Measure the remaining SSB01-S1 volume.
2. Preserve a small residual SSB01-S1 backup if practical.
3. Record:
   - OD750;
   - microscopy;
   - pigmentation;
   - clumping;
   - overall culture appearance.
4. Divide the harvest volume among 50 mL tubes.
5. Centrifuge using the validated harvest condition.
6. Record:
   - total culture volume harvested;
   - number of tubes;
   - pellet appearance;
   - wet pellet mass, if measured.
7. Allocate pellets according to the downstream plan:
   - fresh extraction;
   - flash-frozen protein/pigment pellet;
   - DNA archive.
8. Store immediately at −80°C or proceed directly with downstream processing.

---

# Incubation

## Liquid cultures

Maintain the new liquid cultures under the current validated conditions unless a deliberate variable is being tested.

- Temperature: **26°C**
- Photoperiod: **12 h light : 12 h dark**
- Light intensity at flask level: approximately **40 µmol photons m⁻² s⁻¹**
- Closures: sterile breathable gauze
- Maintain adequate flask spacing and headspace.

Record the actual light intensity across incubator positions with the chamber closed.

## F2P1 plates

- Temperature: **24–26°C**
- Photoperiod: **12 h light : 12 h dark**
- Light intensity: approximately **20–30 µmol photons m⁻² s⁻¹**
- Protect plates from drying.
- Inspect without repeatedly opening plates.

---

# Monitoring schedule

## Day 0

- [ ] Photograph every flask.
- [ ] Record source and final volumes.
- [ ] Record OD750.
- [ ] Perform microscopy.
- [ ] Confirm labels.
- [ ] Record incubator positions.
- [ ] Photograph F2P1 plates after inoculation.

## Days 2–3

- [ ] Check liquid cultures for contamination.
- [ ] Check sedimentation and clumping.
- [ ] Confirm that M1 cultures remain pigmented and intact.
- [ ] Inspect F2P1 plates for drying or contamination.

## Day 7

- [ ] Measure OD750.
- [ ] Perform microscopy.
- [ ] Photograph all cultures.
- [ ] Compare S2 growth with corresponding M1 and S1 records.
- [ ] Inspect and photograph F2P1 plates.

## Weekly thereafter

- [ ] Measure OD750.
- [ ] Record culture appearance.
- [ ] Perform microscopy.
- [ ] Record F2P1 colony development.
- [ ] Check incubator temperature and light.
- [ ] Record evaporation or volume loss.

---

# Acceptance criteria

## M1 success

An M1 culture is accepted as the maintenance replacement when:

- cells remain visibly intact;
- chlorophyll pigmentation is retained;
- microscopy shows living cells;
- no obvious contamination is detected;
- cell abundance or pigmentation is stable or increasing.

Do **not** discard the corresponding S1 culture until successful M1 establishment has been confirmed.

## S2 success

An S2 culture is considered suitable for continued biomass production when:

- pigmentation is retained after transfer;
- microscopy supports active growth;
- no obvious contamination is present;
- culture density or visible biomass increases over time.

## F2P1 colony recovery

Successful plating is indicated by:

- discrete pigmented colonies;
- minimal contamination;
- colonies that can be transferred into fresh liquid F/2.

If no colonies develop after several weeks, reassess:

- inoculum density;
- agar preparation;
- plate moisture;
- illumination;
- incubation temperature;

before considering Marine Broth contamination screening.

---

# Inventory summary

## Core experiment only

| Item | Required |
|---|---:|
| Fresh liquid 1× F/2 | 4.0 L |
| F/2 agar | 300 mL |
| 50× F/2 stock (total) | 86 mL |
| Sterile ASW (total) | 4.214 L |
| 250 mL Erlenmeyer flasks | 5 |
| 2 L Erlenmeyer flasks | 5 |
| Petri plates | 12 |
| Dilution tubes | 10 |
| Archive cryovials | 15 |
| 15 mL tubes | 15 |
| 50 mL tubes | 2–3 |

## Core experiment plus optional IMK module

| Item | Required |
|---|---:|
| Sterile ASW (minimum total) | 4.655 L |
| 50× F/2 stock | 86 mL |
| 50× IMK stock | 9 mL |
| Casein hydrolysate | 0.60 g |
| Glucose | 0.50 g |
| 250 mL flasks | 14 total |
| 2 L flasks | 5 |
| Total new liquid-culture flasks | 19 |
| F/2 agar plates | 12 |

---

# Final execution checklist

## Before beginning

- [ ] S1 cultures inspected by microscopy.
- [ ] Source culture volumes measured.
- [ ] OD750 recorded.
- [ ] Glassware cleaned.
- [ ] Flasks and gauze sterilized.
- [ ] 4.0 L liquid F/2 prepared.
- [ ] 300 mL F/2 agar prepared and poured.
- [ ] All flasks, tubes, plates, and cryovials labeled.
- [ ] Incubator space confirmed.

---

## Experimental workflow

- [ ] Establish all M1 cultures.
- [ ] Establish all S2 cultures.
- [ ] Prepare all F2P1 dilution plates.
- [ ] Prepare A1 analytical pellet archives.
- [ ] Retain residual S1 cultures as temporary backups.
- [ ] Harvest SSB01 only after M1 and S2 have been established.

---

## End of Day 0

- [ ] Photograph all liquid cultures.
- [ ] Photograph all plates.
- [ ] Record incubator positions.
- [ ] Record all inoculation volumes.
- [ ] Record plate IDs.
- [ ] Record archive tube IDs.
- [ ] Record freezer location of A1 pellets.
- [ ] Record laboratory notebook entry.

---

# Expected outputs

At completion of the protocol, the collection should consist of:

| Material | Quantity |
|---|---:|
| S1 backup cultures | 5 |
| M1 maintenance cultures | 5 |
| S2 biomass cultures | 5 |
| F2P1 plates | 12 |
| A1 archive pellets | 15 |
| H1 pilot harvest | 1 (SSB01 only) |

---

# Notes

- M1 is the priority branch and should always be established first.
- S1 cultures should be retained until M1 and S2 have clearly recovered.
- F2P1 provides a colony-derived backup independent of the liquid cultures.
- A1 is intended solely as a frozen analytical archive and is **not** a cryopreservation protocol.
- H1 is reserved for the initial SSB01 biomass harvest and downstream extraction tests.